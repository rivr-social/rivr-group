 ...
 * an actor cannot set on themselves via profile edit or other metadata write
 * paths.
 *
 ...
 *
 * Exported so any metadata writer can strip these reserved keys from any
 * metadata a caller tries to persist (DBR-SEC-002).
 */
...
export async function check(
  ...
  // ABAC deny policies take precedence over every allow path below. Evaluate
  // them up front so an explicit deny short-circuits owner/grant/visibility.
  ...
  // 9. ABAC policy evaluation — an allow policy (evaluated up front) grants access.
 ...
 * A matching deny policy short-circuits and wins over any allow policy.
 */
